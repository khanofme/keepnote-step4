...
public class NoteServiceImpl implements NoteService{
	...
	public boolean deleteNote(String userId, int noteId) {
		...
		if(noteUser.get()!=null){
			List<Note> noteList=noteUser.get().getNotes();
			for(Note note:noteList) {
				if(note.getNoteId()==noteId) {
					noteUser.get().getNotes().remove(note);
					return true;
				}
			}
		}
	...
	public Note updateNote(Note note, int id, String userId) throws NoteNotFoundExeption {
		...
		try {
		Note foundNote=getNoteByNoteId(userId,id);
	    int index=noteRepository.findById(userId).get().getNotes().indexOf(foundNote);
	    noteRepository.findById(userId).get().getNotes().add(index, foundNote);
